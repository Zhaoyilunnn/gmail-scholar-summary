...
class ACMFetcher(PaperFetcher):
    """ACM Digital Library HTML 页面解析器."""
    ...
    def can_fetch(self, url: str) -> bool:
        """检查是否可以处理 ACM Digital Library DOI 页面.
        ...
        return host == "dl.acm.org" and path.startswith("/doi/")

    def fetch(self, url: str) -> PaperInfo:
        """从 ACM Digital Library URL 获取论文信息.
        ...
        if not self.can_fetch(url):
            raise PaperFetchError(f"不支持的 ACM URL: {url}")
        ...
        for attempt in range(self.retry_times):
            try:
                ...
                raise PaperFetchError(f"获取 ACM 论文失败: {url} - {last_error}")

        raise PaperFetchError(f"获取 ACM 论文失败，已重试 {self.retry_times} 次: {url}")
    ...
    def _parse_html(self, html: str, url: str) -> PaperInfo:
        ...
        ) or get_text_by_selectors(soup, ["h1.citation__title", "h1"])
        ...
        abstract = get_meta_content(
            ...
            soup,
            [
                "section#abstract .abstractSection",
                ".abstractSection",
                "div.abstractInFull",
            ],
        )
        ...
        venue = normalize_whitespace(venue) or "ACM"

        if not title:
            raise PaperFetchError(f"无法从 ACM 页面提取标题: {url}")
        if not abstract:
            raise PaperFetchError(f"无法从 ACM 页面提取摘要: {url}")
        if is_blocked_text(title) or is_blocked_text(abstract):
            raise PaperFetchError(f"ACM 页面疑似反爬或验证码页面: {url}")
    ...
    def _clean_title(self, title: str) -> str:
        ...
        title = re.sub(r"\s*\|\s*ACM.*$", "", title, flags=re.IGNORECASE)
        return title
